...
constructFlinkClassPathMore() {
    local FLINK_DIST
    local FLINK_CLASSPATH="" # Initialize to empty string

    # Defensive check if FLINK_LIB_DIR is set
    if [ -z "$FLINK_LIB_DIR" ] || [ ! -d "$FLINK_LIB_DIR" ]; then
        >&2 echo "[ERROR] FLINK_LIB_DIR ($FLINK_LIB_DIR) is not set or not a directory in constructFlinkClassPathMore."
        exit 1 # Or return empty / handle error
    fi

    # --- Process FLINK_LIB_DIR ---
     while read -d '' -r jarfile ; do
        if [[ "$jarfile" =~ .*/flink-dist[^/]*.jar$ ]]; then
            if [ -z "$FLINK_DIST" ]; then FLINK_DIST="$jarfile"; else FLINK_DIST="$FLINK_DIST:$jarfile"; fi
        elif [[ "$FLINK_CLASSPATH" == "" ]]; then FLINK_CLASSPATH="$jarfile"; else FLINK_CLASSPATH="$FLINK_CLASSPATH":"$jarfile"; fi
    done < <(find "$FLINK_LIB_DIR" ! -type d -name '*.jar' -print0 | sort -z)
    # --- End Processing FLINK_LIB_DIR ---

    # <<< --- START: Append Custom Jars --- >>>
    local ALL_CUSTOM_JARS_LIST=""

    # --- Use process substitution to avoid subshell for the 'while' loop ---
    while IFS= read -r single_dir; do
        # Trim potential leading/trailing whitespace from the path
        single_dir=$(echo "$single_dir" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//' -e 's/^"//' -e 's/"$//'); if [ -z "$single_dir" ]; then continue; fi

        if [ -d "$single_dir" ]; then
            local JARS_IN_THIS_DIR; JARS_IN_THIS_DIR=$(find "$single_dir" -maxdepth 1 -name '*.jar' -print0 | paste -sd: -z | tr -d '\0')
            if [ -n "$JARS_IN_THIS_DIR" ]; then
                # Now modifications to ALL_CUSTOM_JARS_LIST persist outside the loop
                if [ -n "$ALL_CUSTOM_JARS_LIST" ]; then
                    ALL_CUSTOM_JARS_LIST="$ALL_CUSTOM_JARS_LIST:$JARS_IN_THIS_DIR"
                else
                    ALL_CUSTOM_JARS_LIST="$JARS_IN_THIS_DIR"
                fi
                 >&2 echo "[INFO] Added custom JARs from $single_dir" # Debug message
            fi
        else >&2 echo "[WARNING] Custom JAR directory listed in string not found: $single_dir"; fi
    done < <(echo "$CUSTOM_JARS_DIRS" | tr ';' '\n') # <<<--- CHANGE HERE

    # Now this check should work correctly
    if [ -n "$ALL_CUSTOM_JARS_LIST" ]; then
        if [ -n "$FLINK_CLASSPATH" ]; then
            FLINK_CLASSPATH="$FLINK_CLASSPATH:$ALL_CUSTOM_JARS_LIST"
        else
            FLINK_CLASSPATH="$ALL_CUSTOM_JARS_LIST"
        fi
    fi
    # <<< --- END: Append Custom Jars --- >>>

    # --- Original flink-dist Check ---
    local FLINK_DIST_COUNT=0
    if [ -n "$FLINK_DIST" ]; then FLINK_DIST_COUNT=$(echo "$FLINK_DIST" | tr ':' '\n' | grep -c .); fi # Count non-empty lines
    if [[ "$FLINK_DIST_COUNT" -eq 0 ]]; then (>&2 echo "[ERROR] Flink distribution jar not found in $FLINK_LIB_DIR."); exit 1;
    elif [[ "$FLINK_DIST_COUNT" -gt 1 ]]; then (>&2 echo "[ERROR] Multiple flink-dist*.jar found in $FLINK_LIB_DIR:"); (>&2 echo "$FLINK_DIST" | tr ':' '\n'); exit 1; fi
    # --- End flink-dist Check ---

    if [ -n "$FLINK_CLASSPATH" ]; then echo "$FLINK_CLASSPATH:$FLINK_DIST"; else echo "$FLINK_DIST"; fi
}
